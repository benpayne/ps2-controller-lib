// PS/2 Keyboard Decoder Core Module
//
// Parameterized, portable PS/2 protocol decoder
// Based on ttgf-ps2-m68k implementation (tested to 95% coverage)
//
// Author: Ben Payne, 2024
// License: Apache-2.0

module ps2_decoder_core #(
    ...
    output wire       interrupt,  // Sticky interrupt flag (cleared by int_clear)
    output wire [7:0] data        // Decoded scan code byte
);

    localparam SYSTEM_CLOCK = CLK_FREQ_HZ;
    localparam PS2_CLOCK = PS2_CLK_HZ;
                ...
                if (clk_timeout >= PS2_BIT_TIME[12:0]) begin
                    case(state_reg)
                        IDLE: begin
                            ...
                            ps2_value <= {shift_reg[2], shift_reg[3], shift_reg[4], shift_reg[5],
